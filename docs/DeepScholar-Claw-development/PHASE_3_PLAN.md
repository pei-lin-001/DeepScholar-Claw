...
| DONE | DeepScholar CLI 接入                   | `pnpm deepscholar -- research runner smoke/status/abort` 可用 |
...
| DONE | DeepScholar CLI 增加 runner list     | `pnpm deepscholar -- research runner list --project-id p1 [--json]` 可用 |
...
| DONE | DeepScholar CLI 接入 collect | `pnpm deepscholar -- research runner collect --project-id --run-id` |
...
| DONE | DeepScholar CLI 接入 experiment run    | `pnpm deepscholar -- research experiment run --project-id p1 [--json]` 可用 |
...
pnpm deepscholar -- research runner status \
  --project-id p1 \
  --run-id "<runId>" \
  --home "$HOME_DIR" \
  --json
...
pnpm deepscholar -- research runner abort \
  --project-id p1 \
  --run-id "<runId>" \
  --home "$HOME_DIR" \
  --json
