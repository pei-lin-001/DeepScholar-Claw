...
现在 Phase 1 的成果是：你可以用 OpenClaw 的 CLI 直接驱动一条可执行链路，把“找论文”变成可重复、可审计的工程动作。
...
| 文献模块 CLI 命令                    | DONE | `src/cli/research-cli.ts`（`openclaw research literature ...`）                                   |
| 单元测试 + 集成测试                  | DONE | `services/paper-intel/src/*.test.ts` + `src/cli/program/register.subclis.test.ts`                 |
...
```bash
openclaw research literature search --source semantic-scholar --query "graph rag" --limit 5
openclaw research literature search --source openalex --query "graph rag" --limit 5 --json
```
...
```bash
openclaw research literature ingest --project-id demo --source semantic-scholar --query "graph rag" --limit 20
```
...
```bash
openclaw research literature query --project-id demo --query "graph" --limit 5
openclaw research literature query --project-id demo --query "graph" --limit 5 --json
```

### 4) PDF -> TEI（GROBID）

```bash
openclaw research literature grobid --project-id demo --paper-id P1 --pdf ./paper.pdf --grobid-url http://127.0.0.1:8070
```
...
openclaw research literature graph-build --project-id demo
openclaw research literature query --project-id demo --query "graph" --graph-backend neo4j
```
