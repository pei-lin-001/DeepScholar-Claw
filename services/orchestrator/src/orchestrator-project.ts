...
export async function completeProject(
  ...
  const project = await deps.projects.load(input.projectId);
  if (project.step !== "step12_human_final") {
    throw new Error(`当前步骤不是 step12_human_final，不能完成项目: step=${project.step}`);
  }
  if (!project.gates.reviewCompleted) {
    throw new Error("评审尚未完成（reviewCompleted=false），不能完成项目");
  }
  ...
  return await checkpointAndSave({
    ...
    auditAction: "project.complete",
    details: { input: input.summary, output: "completed" },
    auditActor: actor(input.actorId ?? "human", "human"),
  });
}
