...
function registerLiteratureCli(research: Command) {
  ...
  registerLiteratureDownloadPdf(literature);
...
function registerLiteratureDownloadPdf(literature: Command) {
  literature
    .command("download-pdf")
    .description("Download open-access PDF for an ingested paper into literature/pdfs")
    .requiredOption("--project-id <id>", "Project id")
    .requiredOption("--paper-id <id>", "Paper id")
    .option("--home <dir>", "Override DeepScholar home directory (default: ~/.deepscholar)")
    .option("--json", "Output JSON", false)
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, () => runLiteratureDownloadPdf(opts));
    });
}

async function runLiteratureDownloadPdf(opts: Record<string, unknown>) {
  ...
  defaultRuntime.log(`PDF 已下载 | ${result.pdfPath}`);
  defaultRuntime.log(`校验信息 | sha256=${result.sha256} | bytes=${result.bytes}`);
}
