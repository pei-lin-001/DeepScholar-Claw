...
export function registerResearchOrchestratorCli(
  ...
  registerResearchComplete(research, runtime);
}
...
function registerResearchComplete(research: Command, runtime: CliRuntime): void {
  research
    .command("complete")
    .description("Complete a project at Step12 and write an audit trail entry")
    .requiredOption("--project-id <id>", "Project id")
    .requiredOption("--summary <text>", "Final summary for the project handoff")
    .option("--home <dir>", "Override DeepScholar home directory (default: ~/.deepscholar)")
    .option("--actor-id <id>", "Audit actor id", "human")
    .option("--json", "Output JSON", false)
    .action(async (opts) => {
      await runCommandWithRuntime(runtime, () => runResearchComplete(opts, runtime));
    });
}

async function runResearchComplete(
  opts: Record<string, unknown>,
  runtime: CliRuntime,
): Promise<void> {
  const deps = createOrchestratorDeps(opts.home as string | undefined);
  const project = await completeProject(deps, {
    projectId: parseNonEmptyText(opts.projectId, "project-id"),
  ...
  printJsonOrSummary(
    opts,
    project,
    `项目已完成 ${project.projectId} | 状态 ${project.lifecycle} | 当前步骤 ${project.step}`,
    runtime.log,
  );
}
